	...
	A: Into<OsString> + Clone,
{
	...
	let matches = app.clone().try_get_matches_from(argv);

	if matches.is_err()
	{
		let (mut stdout, mut stderr) = create_io_streams(None);
		return handle_clap_error(
			matches.unwrap_err(),
			app.clone(),
			&mut stdout,
			&mut stderr,
	...
	E: WriteColor,
{
	...
	{
		Some(("run", sub_matches)) => invoke_internal_compiler(
...
///
/// The exit code.
fn invoke_internal_compiler<I, O, E>(
	...
	E: WriteColor,
{
	...
	invoke_compiler(source.unwrap(), &mut session_globals)
		.map_err(|error| {
			error.print(&mut session_globals);
			return error.exit_code;
		})
